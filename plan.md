...
> **Clarification — Local-first architecture**
>
> The primary developer/operator workflow is local-first: `docker compose up` bootstraps the local runtime (cc-switch, Fabric, Hermes, scanners and local model runtimes). The portal (`report-viewer`) is a web UI that can be hosted publicly to provide a read-only report view and operator console, but it does not require hosting of the execution components. Local machines run the scanners and agents and upload structured data (via the secure API) to Supabase; the portal reads from Supabase to render reports and triage UIs. Hosting the portal publicly (for convenience) is optional — all sensitive execution (model runtimes, PoC validation, Fabric patterns, Hermes agents) should remain under operator control on local infrastructure unless you intentionally deploy them to production servers.
...
### Optional Portal Hosting (public web view)

The portal (report-viewer) is primarily a UI that displays reports and triage data stored in Supabase. Hosting the portal publicly is optional — it provides a convenient read-only or operator-facing web view for reports generated from data uploaded by local machines. Execution components (cc-switch, Fabric, Hermes, local model runtimes, PoC validation) are local-first and should remain on operator-controlled infrastructure unless you explicitly choose to deploy them.

If you choose to host the portal publicly (e.g., on Vercel or another static hosting provider), treat it as a separate deployment concern. The portal will need access to Supabase (read-only or service-limited credentials) and the usual secrets for integrations; do not co-locate Fabric, Hermes, or model runtimes on the same public host unless you intend to expose them.

Suggested deploy flow for the portal only:

```
GitHub push → GitHub Actions → build portal → deploy static/app to hosting provider
```

Required secrets for portal deployment (if using GitHub Actions):
- `SUPABASE_URL` (read endpoint)
- `SUPABASE_ANON_KEY` or a limited API key (avoid service_role on public hosts)
- `VIEWER_BASIC_AUTH_USER` / `VIEWER_BASIC_AUTH_PASSWORD` (optional; recommended for private previews)
- `REPORT_VIEWER_BASE_URL` (optional)

Do NOT store `SUPABASE_SERVICE_ROLE_KEY` or any agent tokens on public hosts. Those secrets must remain server-side and local to the execution environment.
